 ...
 */
public class DefaultSpecificationHandler implements SpecificationHandler {
  ...
  public <T> T parseClass(@Nonnull Class<T> clazz) {
    ...
    try {
      ...
      // fetch all the specification state aware beans which are involved in this class processing
      // TODO: more complicated logic - we need to also scan methods
...
//        }
//      }
      final List<SpecificationStateAware> stateAwareBeans = injectionContext.getBeans(SpecificationStateAware.class);
      notifyState(stateAwareBeans, SpecificationState.START);
